...
## 9) Gợi ý đọc code cho người mới (theo thứ tự)

1. `README.md` -> nắm cấu trúc tổng.
2. `pages/index.tsx` -> hiểu đăng nhập + link ví.
3. `pages/dashboard.tsx` và `pages/elections/[id].tsx` -> hiểu trải nghiệm voter.
4. `pages/api/voter/cast-vote.ts` -> hiểu logic bỏ phiếu off-chain.
5. `ethereum/contracts/MultiElection.sol` -> hiểu logic on-chain.
6. `pages/admin/index.tsx` + `pages/api/admin/*` -> hiểu vận hành admin.
7. `pages/api/candidate-wallets.ts` + `pages/api/validate-vote.ts` -> hiểu đối soát/kiểm chứng.
