...
import { GetCandidateWallets, GetVoterByID } from "../lib/api";
...
const ResultsPage = () => {
  ...
  const [elections, setElections] = useState<Array<{ id: string; name: string }>>([]);
  const [selectedElectionId, setSelectedElectionId] = useState<string>("");

  const getWallet = async (showLoader = false) => {
    try {
      if (showLoader) setLoading(true);
      const data = await GetCandidateWallets(selectedElectionId || undefined);
      ...
      toast.error("Không thể tải kết quả.", { style: { background: "#111", color: "#fff" } });
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      const me = await GetVoterByID(auth.token);
      const list = Array.isArray(me?.elections)
        ? me.elections
            .map((x: any) => ({
              id: String(x?.election?._id || x?.election?.id || ""),
              name: String(x?.election?.name || x?.election?.title || "Cuộc bầu cử"),
            }))
            .filter((x: any) => x.id)
        : [];
      setElections(list);
      const qElection = typeof router.query.electionId === "string" ? router.query.electionId : "";
      setSelectedElectionId(qElection || list?.[0]?.id || "");
    })().catch(() => {
      // ignore
    });
  }, [auth?.token, router.query.electionId]);
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl md:text-4xl font-display font-bold text-white">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-accent">Kết quả</span> trực tiếp
              </h1>
              <p className="text-sm text-muted mt-1 flex items-center gap-2">
                <span className="w-2 h-2 bg-success rounded-full animate-pulse" />
                Tự động cập nhật mỗi 5 giây · Lần cuối {lastRefresh.toLocaleTimeString()}
              </p>
            </div>
            <button
          ...
          {elections.length > 0 && (
            <div className="glass-card p-4">
              <p className="text-xs text-muted uppercase tracking-wider font-semibold mb-2">Cuộc bầu cử</p>
              <select
                value={selectedElectionId}
                onChange={(e) => setSelectedElectionId(e.target.value)}
                className="w-full bg-transparent border border-border/60 rounded-xl px-4 py-2.5 text-sm text-white outline-none"
              >
                {elections.map((e) => (
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { label: "Tổng phiếu", value: totalVotes, icon: "🗳️", color: "text-white" },
                  { label: "Ứng viên", value: sorted.length, icon: "👤", color: "text-white" },
                  {
                    label: "Dẫn đầu",
              ...
              {leader && leader.tokens > 0 && (
                <div className="glass-card p-6 md:p-8 relative overflow-hidden border-primary/30">
                  ...
                  <div className="relative z-10 flex flex-col sm:flex-row items-center gap-6">
                    ...
                    <p className="text-xs text-yellow-400 uppercase tracking-widest font-semibold mb-1">Đang dẫn đầu</p>
                    ...
                    <div className="text-center flex-shrink-0">
                      ...
                      <p className="text-xs text-muted mt-1">phiếu ({getPercent(leader.tokens)}%)</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="glass-card overflow-hidden">
                <div className="px-6 py-4 border-b border-border/50 flex items-center justify-between">
                  <h2 className="font-display font-semibold text-white">Tất cả ứng viên</h2>
                ...
                {sorted.length === 0 ? (
                  <div className="p-16 flex flex-col items-center gap-4 text-center">
                    ...
                    <p className="font-display font-semibold text-white">Chưa ghi nhận phiếu bầu</p>
                    <p className="text-sm text-muted">Kết quả sẽ hiển thị khi có phiếu bầu.</p>
                  </div>
                ) : (
              ...
              <div className="glass-card p-4 flex items-start gap-3 border-l border-l-primary/50">
                ...
                <div>
                  <p className="text-sm font-semibold text-white">Kết quả đối soát</p>
                  <p className="text-xs text-muted mt-1">
                    Số phiếu được tổng hợp từ dữ liệu lưu trữ và giao dịch, giúp minh bạch và khó bị thay đổi.
                  </p>
                </div>
              </div>
            </>
