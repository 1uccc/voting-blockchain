...
export default async function handler(
  ...
  try {
    ...
    const electionDoc = await fs.collection("elections").doc(electionId).get();
     ...
     */
    const candidatesSnap = await fs
      .collection("candidates")
      .where("electionId", "==", electionId)
      .where("active", "==", true)
      .get();
    const candidates = candidatesSnap.docs.map((d) => d.data() as any);
