...
const Navbar = () => {
  ...
  const handleLogout = () => {
    setAuth({ name: "", email: "", id: "", role: "", isLoggedIn: false, token: "", locationId: "" });
    toast("Đã đăng xuất.", { style: { background: "#111", color: "#888" } });
    router.push("/");
  };

  const displayName = auth?.name || "Người dùng";
  ...
  const navLinks = [
    ...
    { label: "Kết quả", href: "/candidates" },
    { label: "Xác minh phiếu", href: "/validate" },
  ];
  const adminLinks = [{ label: "Quản trị", href: "/admin" }];
  ...
  return (
    <header>
      ...
      <nav className="hidden md:flex items-center gap-1">
        {(auth?.role === "admin" ? adminLinks : navLinks).map((link) => (
          <Link
            key={link.href}
            href={link.href}
